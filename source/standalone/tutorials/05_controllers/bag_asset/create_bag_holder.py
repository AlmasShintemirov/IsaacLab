...
def create_bag_holder(scene):
    ...
    #add a bag holder 
    holder_path = "/World/envs/env_0/Holder"
    physicsUtils.add_rigid_box(stage,holder_path,size = 0.04,position=(0.57,-0.36,0.4),density=0.0,color=(0.0,0.3,0.7))


    # adding an attachment of the bag handle to the holder
    holder_attachment_path = holder_path + "/attachment"
    holder_attachment = PhysxSchema.PhysxPhysicsAttachment.Define(stage, holder_attachment_path)
    holder_attachment.GetActor0Rel().SetTargets([holder_path])
    holder_attachment.GetActor1Rel().SetTargets([bag_mesh_path])
    PhysxSchema.PhysxAutoAttachmentAPI.Apply(holder_attachment.GetPrim())
